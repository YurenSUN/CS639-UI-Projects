...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    // change back to the original color after 2 seconds
    this.interval = setInterval(() => this.resetBg(), 2000);
  }
    ...
    return (
      <React.Fragment>
        ...
        <Tabs defaultActiveKey="search_cart" style={{ zIndex: 1, width: '100%', backgroundColor: 'white' }}>
          ...
          <Tab eventKey="Completed" title="Completed Courses and Rating">
            <div class="p-2 container-fluid m-0 overflow-auto bg-light" style={{height: "90vh"}}>
              <h3><strong>Completed Courses and Rating</strong></h3>
              <CompletedCourseArea courses={this.state.allCourses} compleCourses={this.state.compleCourses} rating={this.rating.bind(this)} />
            </div>
          </Tab>

          <Tab tabClassName={this.state.recomTabHL ?  "highlighted": ""} id="recomTab" title="Recommended Courses" eventKey="Recommended" inkBarStyle={{background: 'blue'}}>
            <div class="p-2 container-fluid m-0 overflow-auto bg-light" style={{height: "90vh"}}>
